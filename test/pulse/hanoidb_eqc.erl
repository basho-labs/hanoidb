%%% File        : hanoidb_eqc.erl
%%% Author      : Ulf Norell, Hans Svensson, Scott, Greg
%%% Description : HanoiDB stress testing
...
command(S) ->
  frequency(
    ...
    [ {1, {call, ?MODULE, kill, [elements([hanoidb_merge_worker|S#state.readers])]}}
      || S#state.is_writer, S#state.handle /= undefined ] ++
...
precondition(S, {call, _, kill, [Pid]}) ->
  ...
  (Pid == hanoidb_merge_worker orelse lists:member(Pid, S#state.readers));
...
run_commands_on_node(Cmds, Seed, Verbose) ->
  mute(Verbose, fun() ->
    ...
    pulse:start(),
    hanoidb_nifs:set_pulse_pid(utils:whereis(pulse)),
...
prop_pulse_test_() ->
  {timeout, 1000000,
   fun() ->
       copy_hanoidb_app(),
       ?assert(eqc:quickcheck(eqc:testing_time(600,?QC_OUT(prop_pulse()))))
   end}.

%% Needed since rebar fails miserably in setting up the .eunit test directory
copy_hanoidb_app() ->
  os:cmd("cp ../ebin/hanoidb.app ."),
  ok.
...
command_data({set, _, {call, _, kill, [Pid]}}, {_S, _V}) ->
  case Pid of
    hanoidb_merge_worker -> {kill, merger};
    _                    -> {kill, reader}
  end;
...
get(H, K) ->
  ?LOG({get, H, K},
  ?CHECK_HANDLE(H, not_found, hanoidb:get(H, <<K:32>>))).

put(H, K, V) ->
  ?LOG({put, H, K, V},
  ?CHECK_HANDLE(H, ok, hanoidb:put(H, <<K:32>>, V))).
...
delete(H, K) ->
  ?LOG({delete, H, K},
  ?CHECK_HANDLE(H, ok, hanoidb:delete(H, <<K:32>>))).

fork_merge(H) ->
  ?LOG({fork_merge, H},
  ?CHECK_HANDLE(H, not_needed,
  case hanoidb:needs_merge(H) of
    {true, Files} -> catch hanoidb_merge_worker:merge(?BITCASK, [], Files);
    false         -> not_needed
  end)).
...
merge(H) ->
  ...
  case hanoidb:needs_merge(H) of
    {true, Files} ->
      case catch hanoidb:merge(?BITCASK, [], Files) of
        {'EXIT', Err} -> Err;
        R             -> R
      end;
    false -> not_needed
...
needs_merge(H) ->
  ?LOG({needs_merge, H},
  ?CHECK_HANDLE(H, false, hanoidb:needs_merge(H))).
...
sync(H) ->
  ?LOG({sync, H},
  ?CHECK_HANDLE(H, ok, hanoidb:sync(H))).

fold(H) ->
  ?LOG({fold, H},
  ?CHECK_HANDLE(H, [], hanoidb:fold(H, fun(<<K:32>>, V, Acc) -> [{K,V}|Acc] end, []))).

fold_keys(H) ->
  ?LOG({fold_keys, H},
  ?CHECK_HANDLE(H, [], hanoidb:fold_keys(H, fun(#hanoidb_entry{key = <<K:32>>}, Ks) -> [K|Ks] end, []))).

bc_open(Writer) ->
  ?LOG({open, Writer},
  case Writer of
    true  -> catch hanoidb:open(?BITCASK, [read_write, {max_file_size, ?FILE_SIZE}, {open_timeout, 1}]);
    false -> catch hanoidb:open(?BITCASK, [{open_timeout, 1}])
  end).

bc_close(H)    ->
  ?LOG({close, H},
  ?CHECK_HANDLE(H, ok, hanoidb:close(H))).
